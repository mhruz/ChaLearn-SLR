...
class HpoesTransformer(Chain):
    ...
    def decode_prediction(self, x):
        ...
        #return F.argmax(F.softmax(x, axis=1), axis=1)
    ...
    def predict(self, x):
        ...
        prediction = self.model.encode(x, None)  #encode() je to same co __call__() tj. volani model() ???... nevim jaky je tam zasadni rozdil ... mozna gradienty ... ne asi je to jedno
        
        #prediction = self.classifier(prediction, n_batch_axes=2) #klasifikece pro vsechny, stary zpusob
        prediction = self.classifier(prediction[:,0]) #klasifikece pro jednoho, novy zpusob
