...
class MultiHeadedAttention(Chain):
    ...
    def __init__(self, num_heads, size, dropout_ratio=0.1):
        ...
        with self.init_scope():
            self.linears = L.Linear(size, size, initialW=initializers.GlorotUniform()).repeat(4, mode='init') #Repeats this link multiple times to make a Sequential, 4 je asi OK
    ...
    def __call__(self, query, key, value, mask=None):
        ...
        query, key, value = [self.project(linear, x, batch_size) for linear, x in zip(self.linears, (query, key, value))] #prozene query, key a value pres prvni 3 Sequential, ten 4. je vystup
